...
def main(num: int, max_workers: int = 10):
    # 1. 併發新增規則
    results = []
    with ThreadPoolExecutor(max_workers=max_workers) as pool:
        futures = {pool.submit(post_rule, i+1): i+1 for i in range(num)}
        for future in as_completed(futures):
            idx, data = future.result()
            print(f"Rule #{idx} response:", json.dumps(data, ensure_ascii=False))

    # 2. 列出所有規則
    print("\n=== Current rule set ===")
    import time
    time.sleep(2)  # 等待所有規則處理完成
    rules = fetch_all_rules()
    print(json.dumps(rules, indent=2, ensure_ascii=False))

if __name__ == "__main__":
    # max_workers 可依需求調整，不要超過 API 可接受的並發上限
    main(NUM, max_workers=WORKER)
